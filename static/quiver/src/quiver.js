...
class Quiver {
    ...
    export(format, settings, options, definitions) {
        switch (format) {
            ...
            case "database":
                return QuiverImportExport.database.export(this, settings, options, definitions);
            default:
...
QuiverImportExport.database = new class extends QuiverImportExport {
    ...
    export(quiver, _, options) {
        ...
        return {
            ...
            url_data: `${URL_prefix}?q=${
              btoa(unescape(encodeURIComponent(json_string)))
            }${macro_data}`,
    ...
    import(ui, string) {
        ...
        try {
            /// We use this `decodeURIComponent`-`escape` trick to encode non-ASCII characters.
            const decoded = string; /// from base64 code: decodeURIComponent(escape(atob(string)));
            if (decoded === "") {
                return quiver;
            }
            input = JSON.parse(decoded);
